...
# 3. Replace Stack with Box (gap:24) and remove the Button's sx prop
...
# Remove the Button's sx prop (which is sx={{ mt: 2 }})
new_content = new_content.replace('sx={{ mt: 2 }}', '')

with open('src/app/dashboard/page.tsx', 'w') as f:
    f.write(new_content)

print('Fixed: quotes, subscription hook, Stack->Box(gap=24), removed Button sx')
